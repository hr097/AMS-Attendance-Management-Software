...
//#endif  // user 1  file edit settings
...
public:
  APP()
  {
    ...
    CHAR pathDocument[MAX_PATH];                                                                    
    HRESULT result = SHGetFolderPath(NULL, CSIDL_PERSONAL, NULL, SHGFP_TYPE_CURRENT, pathDocument); 
  ...
  ~MODULE_2()
  {
    ...
    if(month_report_flag) // ? according to 1 month trigger
    {
      ...
      if (createMonthlyReportPDF(SemPath + "\\REPORTS\\fac_data.txt", SemPath + "\\REPORTS\\stud_name.txt", SemPath + "\\REPORTS\\stud_att.txt", pdfName))
      {
        ...
        MODULE_2 MD2;
        thread t1(&sendToEmail, MD2, "ams.software.team@gmail.com", "__your_software_email_password__", FacultyEmail, "MONTHLY-ATTENDANCE-REPORT", "Dear Sir/Madam, \nGreetings From Team AMS. \n\nKindly Go throgh Your MONTHLY Attendance Report.\n\nThank You.\n\n", SemPath + "\\REPORTS\\" + pdfName, pdfName);
        ...
        time = time.substr(0,10); // re-used as semester termination date
          ...
          if(confirmSemReport(date,time)) // basic confirmation of semester end
          {
                 ...
                 {   
                     ...
                     //*threading used for processing email part
                     MODULE_2 MD2;
                     thread t1(&sendToEmail, MD2, "ams.software.team@gmail.com", "__your_software_email_password__",tempStorage, "SEMESTER ATTENDANCE REPORT", "Dear Sir/Madam, \nGreetings From Team AMS. \n\nKindly Go throgh Your Semester Attendance Report.\n\nThank You.\n\n", SemPath + "\\REPORTS\\" + pdfName, pdfName);
                     thread t2(&LoadingProcess, MD2);
                     t1.join(); 
                     t2.join(); 
                     scrClr(); 
    ...
    if(checkEmptyFile(SemPath + "\\DAILY-RECORD\\records.txt"))
    {
        if (DateInput())
        {
            ...
            if (createDateWiseReportPDF(SemPath + "\\REPORTS\\fac_data.txt", SemPath + "\\REPORTS\\stud_name.txt", SemPath + "\\REPORTS\\stud_att.txt", pdfName))
            {
              ...
              MODULE_3 MD3;
              thread t1(&sendToEmail, MD3, "ams.software.team@gmail.com", "__your_software_email_password__", FacultyEmail, "CUSTOMIZE-ATTENDANCE-REPORT", "Dear Sir/Madam, \nGreetings From Team AMS. \n\nKindly Go throgh Your Customized Attendance Report.\n\nThank You.\n\n", SemPath + "\\REPORTS\\" + pdfName, pdfName);
        ...
        if (student_email.length() > 29)
        {
          if (studConfirmation(1))
          {
            ...
            if (createStudentWiseReportPDF(SemPath + "\\REPORTS\\stud_data.txt", SemPath + "\\REPORTS\\stud_date.txt", SemPath + "\\REPORTS\\stud_time.txt", SemPath + "\\REPORTS\\stud_att.txt", pdfName))
            {
              ...
              MODULE_3 MD3;
              thread t1(&sendToEmail, MD3, "ams.software.team@gmail.com", "__your_software_email_password__", tempStorage, "CUSTOMIZE-ATTENDANCE-REPORT", "Dear Sir/Madam, \nGreetings From Team AMS. \n\nKindly Go throgh Your Customized Attendance Report.\n\nThank You.\n\n", SemPath + "\\REPORTS\\" + pdfName, pdfName);
              thread t2(&LoadingProcess, MD3);
...
//  system("cls");   // ?  console file security : unauthorised user acess prevention
//  if( (!strcmp(argv[1],"__master__code__for__gui")) && (argc == 2 ) ) 
//  {
    APP A;
 ...
 // }  // console file security : unauthorised user acess prevention
  return(EXIT_SUCCESS);
}